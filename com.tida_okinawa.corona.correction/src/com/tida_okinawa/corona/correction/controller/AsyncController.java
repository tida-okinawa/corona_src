...
abstract public class AsyncController<TS, TR> implements IRunnableWithProgress {
    ...
    AsyncController(String name) {
        this.name = name;
        /*
         * TODO 処理スレッド数の設定をcorrectionに移す
         * そして、uiにあるAsyncControllerをこちらと置き換える。
         */
    ...
    public void run(final IProgressMonitor progressMonitor) throws InterruptedException, InvocationTargetException {
        System.out.println("# create " + nProcThread + " threads."); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        /* 処理スレッドを準備 */
        Thread[] threads = new Thread[nProcThread];

        /* 処理スレッドの起動 */
        for (int i = 0; i < nProcThread; i++) {
            ...
            while ((data = next()) != null && !monitor.isCanceled()) {
                try {
                    ...
                    if (CoronaActivator.isDebugMode()) {
                        status = new Status(IStatus.ERROR, CorrectionActivator.PLUGIN_ID, "Uncaught exception occurred.", e); //$NON-NLS-1$
                    } else {
                        status = new Status(IStatus.WARNING, CorrectionActivator.PLUGIN_ID, "Uncaught exception occurred.", e); //$NON-NLS-1$
                    }
