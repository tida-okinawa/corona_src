/**
 * @version $Id: SynonymDicEditor.java 1840 2014-04-16 05:38:34Z yukihiro-kinjyo $
 * 
 * 2011/10/05 18:33:52
 ...
 * @author kousuke-morishima
 */
public class SynonymDicEditor extends MasterServantEditor2 {
    public static final String EDITOR_ID = "com.tida_okinawa.corona.ui.editor.synonymdic"; //$NON-NLS-1$


    /**
     * 同義語辞書エディターオブジェクトの初期化
     */
    public SynonymDicEditor() {
        super(new int[] { 1, 1 });
    }
    ...
    public void dispose() {
        super.dispose();
        if (isLocalFile()) {
            /* エラーのデータは保存されないので、閉じたらマーカーは消さなきゃいけない */
    ...
    public void doSave(IProgressMonitor monitor) {
        ...
        // Memo 今は、SynonymDicが持っているitemsをそのままもらっているのでこれでOK
        int i = 0;
        ...
        if (errorItemMap.size() > 0) {
            ...
            message.append(Messages.SynonymDicEditor_messageSaveFail);
            int cnt = 0;
            for (Entry<IDepend, Integer> e : errorItemMap.entrySet()) {
            ...
            MessageDialog.openWarning(getSite().getShell(), Messages.SynonymDicEditor_unsavedItem, message.toString());
    ...
    protected void addButtonSelected(String masterOrServant) {
        ...
        if (MASTER.equals(masterOrServant)) {
            /* 単語を選択してもらう */
            d.setTitle(Messages.SynonymDicEditor_titleSelectMainTerm);
            d.setMessage(Messages.SynonymDicEditor_messageSelectMainTerm);
            ...
            if (d.open() == Dialog.OK) {
                ...
                for (Object r : d.getResult()) {
                    /* 辞書に */
                    addedItem = factory.createSynonym((ITerm) r);
            ...
            if (elements.size() == 1) {
                ...
                /* 登録済み単語と親を消す */
                // viewItems.removeAll(registeredSubItems);
                viewItems.remove(parent.getMain());
                d.setElements(viewItems.toArray());

                d.setTitle(Messages.SynonymDicEditor_titleSelectDependentTerm);
                d.setMessage(Messages.SynonymDicEditor_messageSelectDependentTerm);
                d.setValidator(new ISelectionStatusValidator() {
                    ...
                    public IStatus validate(Object[] selection) {
                        ...
                        if (message.length() > 0) {
                            return new Status(IStatus.ERROR, UIActivator.PLUGIN_ID, message.substring(2) + Messages.SynonymDicEditor_messageCycle);
                        }
                        ...
                        if (message.length() > 0) {
                            return new Status(IStatus.ERROR, UIActivator.PLUGIN_ID, message.substring(2) + Messages.SynonymDicEditor_messageRegistered);
                        }
